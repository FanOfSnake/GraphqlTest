...
namespace GraphqlTest.Types
{
    public class AttendeeType: ObjectType<Attendee>
    {
        protected override void Configure(
            IObjectTypeDescriptor<Attendee> descriptor)
        {
            descriptor
                .ImplementsNode()
                .IdField(t => t.Id)
        ...
        private class AttendeeResolvers
        {
            ...
            {
                int[] speakersIds = await dbContext.Attendees
                    .Where(a => a.Id == attendee.Id)
                    .Include(a => a.SessionAttendees)
                    .SelectMany(a => a.SessionAttendees.Select(
                        t => t.SessionId))
                    .ToArrayAsync();

                return await sessionById.LoadAsync(speakersIds, cancellationToken);
            }
        }
    }
